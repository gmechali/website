...
then
  ...
  # Generate continent summaries
  echo "Generating continent summaries"
  python3 -m $MODULE.fetch_place_summaries static/sitemap/Continent.0.txt \
    --output_file $DIR/generated_summaries/continents.json \
  ...
  python3 -m $MODULE.generate_place_summary_shards \
    $DIR/generated_summaries/continents.json
